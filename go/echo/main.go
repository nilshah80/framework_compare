...
func main() {
	...
	orders.PUT("/:orderId", func(c echo.Context) error {
		...
		if pgStore != nil {
			...
			if !found {
				return c.JSON(http.StatusNotFound, map[string]string{"error": "order not found", "order_id": orderID, "request_id": requestID})
			}
	...
	orders.DELETE("/:orderId", func(c echo.Context) error {
		...
		if pgStore != nil {
			...
			return c.JSON(http.StatusOK, map[string]string{"message": "order deleted", "order_id": orderID, "request_id": requestID})
		}
